...
// Header user info injection
// We patch the header to show user avatar + sign out
const _OrigApp = App;
function AppWithAuth() {
const user = window.__fbUser;
const [syncing, setSyncing] = useState(false);
window.__syncDot = (s) => setSyncing(s===“syncing”);

const app = _OrigApp();

// Find header and inject user info — simpler: just render overlay
return (
<div style={{position:“relative”}}>
{app}
{/* User badge fixed top-right */}
<div style={{position:“fixed”,bottom:16,right:16,display:“flex”,alignItems:“center”,gap:8,background:“rgba(44,44,44,0.85)”,borderRadius:20,padding:“6px 12px 6px 6px”,zIndex:999,backdropFilter:“blur(8px)”}}>
{user?.photoURL && <img src={user.photoURL} alt=“avatar” style={{width:24,height:24,borderRadius:“50%”}}/>}
<span style={{fontSize:11,color:”#fff”,maxWidth:120,overflow:“hidden”,textOverflow:“ellipsis”,whiteSpace:“nowrap”}}>{user?.displayName||user?.email}</span>
<div style={{width:6,height:6,borderRadius:“50%”,background:syncing?”#C8956C”:”#6B8F71”,flexShrink:0}}/>
<button onClick={()=>window.__fbSignOut&&window.__fbSignOut()} style={{background:“none”,border:“none”,color:”#aaa”,fontSize:11,cursor:“pointer”,fontFamily:“inherit”,padding:“0 2px”}}>登出</button>
</div>
</div>
);
}
